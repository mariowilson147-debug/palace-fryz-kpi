      ...
      <div className="px-8 py-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 h-44">
          ...
          <div className="bg-white rounded-lg shadow-sm p-3 relative">
            ...
            <ResponsiveContainer width="100%" height="90%"><PieChart><Pie data={expenseCategories} dataKey="value" nameKey="name" innerRadius="65%" outerRadius="85%" isAnimationActive={false} paddingAngle={2}>{expenseCategories.map((_: any, i: number) => <Cell key={i} fill={['#e85d04', '#0d9488', '#3b82f6', '#d4af37', '#9ca3af'][i % 5]} />)}</Pie><Legend layout="vertical" verticalAlign="middle" align="right" wrapperStyle={{fontSize:7}} iconSize={5} /></PieChart></ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-3 h-40">
          ...
          <ResponsiveContainer width="100%" height="85%"><LineChart data={trendData} margin={{left: -10}}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" fontSize={7} axisLine={false} tickLine={false} /><YAxis fontSize={7} tickFormatter={(v)=>`${v/1000}k`} axisLine={false} tickLine={false} /><Line type="monotone" dataKey="sales" stroke="#e85d04" strokeWidth={2} dot={{ r: 2 }} isAnimationActive={false} /><Line type="monotone" dataKey="expenses" stroke="#1e3a8a" strokeWidth={2} dot={false} isAnimationActive={false} /><Line type="monotone" dataKey="waste" stroke="#ef4444" strokeWidth={1} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer>
        </div>

        <div className="grid grid-cols-2 gap-4 h-40">
          <div className="bg-white rounded-lg shadow-sm p-3">
            <h3 className="text-[9px] font-bold text-[#0a192f] mb-2 uppercase">Highest & Lowest Days</h3>
            <ResponsiveContainer width="100%" height="85%"><BarChart data={extremeData} margin={{ left: -20 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" fontSize={7} axisLine={false} tickLine={false} interval={0} /><YAxis fontSize={7} tickFormatter={(v)=>`${v/1000}k`} axisLine={false} tickLine={false} /><Bar dataKey="value" barSize={12} isAnimationActive={false} label={{ position: 'top', fontSize: 6, formatter: ((v: any) => fmt(Number(v))) as any }}>{extremeData.map((d: any, i: number) => <Cell key={i} fill={d.fill} />)}</Bar></BarChart></ResponsiveContainer>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-3">
            <h3 className="text-[9px] font-bold text-[#0a192f] mb-2 uppercase">Waste % By Branch</h3>
            <ResponsiveContainer width="100%" height="85%"><BarChart data={branchesData} margin={{ left: -20 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" fontSize={7} axisLine={false} tickLine={false} /><YAxis fontSize={7} tickFormatter={(v)=>`${v}%`} axisLine={false} tickLine={false} /><ReferenceLine y={10} stroke="#d4af37" strokeDasharray="3 3" label={{ position: 'top', value: '10% Target', fill: '#d4af37', fontSize: 7, fontWeight: 'bold' }} /><Bar dataKey="wasteRate" fill="#ef4444" barSize={12} isAnimationActive={false} label={{ position: 'top', fill: '#ef4444', fontSize: 7, formatter: (v:any)=>`${v}%` }} /></BarChart></ResponsiveContainer>
          </div>
        </div>
