  ...
  const exportPDF = async (ref: React.RefObject<HTMLDivElement | null>, filename: string) => {
    ...
    try {
      const dataUrl = await toPng(ref.current, { 
        ...
        filter: (node) => {
          // html-to-image is very robust but omitting classes that cause issues is sometimes needed.
          return true;
        }
      });
      const pdf = new jsPDF('p', 'mm', 'a4');
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const pdfHeight = (ref.current.offsetHeight * pdfWidth) / ref.current.offsetWidth;
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-8">
            ...
            {branchesData.map((branch) => (
              <div key={branch.id} className="relative">
                <button 
                  onClick={() => exportPDF(branchRefs.current[branch.id] as any, `PFMS_${branch.name.replace(/\s+/g, '_')}_Report`)}
          ...
          <div className="space-y-8">
            ...
            <div className="relative">
              <button 
                onClick={() => exportPDF(comparativeRef as any, `PFMS_Comparative_Report`)}
                className="absolute top-6 right-6 z-10 btn-secondary text-sm hidden md:flex items-center gap-2 group print:hidden hover:border-gold/50"
